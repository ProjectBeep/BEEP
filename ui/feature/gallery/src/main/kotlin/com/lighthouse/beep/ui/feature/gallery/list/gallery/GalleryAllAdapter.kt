...
internal class GalleryAllAdapter(
    ...
    private val onGalleryListener: OnGalleryItemListener,
) : PagingDataAdapter<GalleryItem, RecyclerView.ViewHolder>(GalleryItemDiff()) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType) {
            ...
            TYPE_ITEM -> GalleryViewHolder(parent, requestManager, onGalleryListener)
            else -> throw RuntimeException("${javaClass.simpleName}에 정의 되지 않는 item 입니다")
        }
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {val item = getItem(position)
        when {
